...
amount_qty_transactions AS (
  SELECT
     CAST(q.quarter_start_date AS DATE) AS quarter_start_date
    ,COUNT(DISTINCT ct.card_number) AS qty_card_with_transactions
    ,SUM(ct.amount) AS avg_amount_transactions_quarter
...
SELECT
  ...
  ,ab.quarter
  ,aqt.qty_card_with_transactions
  ,aqt.avg_amount_transactions_quarter
  ,(aqt.avg_amount_transactions_quarter/ aqt.qty_card_with_transactions) AS avg_amount_per_transaction

FROM active_beginning_quarter AS ab
LEFT JOIN amount_qty_transactions AS aqt
  ON ab.quarter_start_date = aqt.quarter_start_date
